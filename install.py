...
def ask_for_venv():
    ...
    if USE_VENV:
        ...
        print("Installing globally or in the current environment (no venv)")
    return USE_VENV
...
def get_pip_path():
    ...
    if USE_VENV:
        return str(Path("venv") / ("Scripts" if IS_WINDOWS else "bin") / ("pip.exe" if IS_WINDOWS else "pip"))
    return sys.executable.replace("python.exe", "pip.exe") if IS_WINDOWS else "pip"

def get_python_path():
    """Return Python path (venv or system)"""
    if USE_VENV:
        return str(Path("venv") / ("Scripts" if IS_WINDOWS else "bin") / ("python.exe" if IS_WINDOWS else "python"))
